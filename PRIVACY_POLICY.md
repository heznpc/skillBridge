...
As of September 1, 2026, the Chrome Web Store still publishes **SkillBridge v1.0.1**. That legacy version includes a bundled Puter client used for Gemini translation review and the Claude-powered AI Tutor, and it declares YouTube host access.
...
These statements describe the implemented data-flow contract and automated regression coverage. This checkpoint does not claim an authenticated live Academy post-submit DOM capture or a final signed-in Puter round trip; both remain final CWS gates.
...
The unpublished source build includes an AI Tutor using Claude — Sonnet 4.6, falling back to Sonnet 4.5 if that model is unavailable — through the bundled Puter client. The client initializes in Chrome's ISOLATED content-script world only on the top-level `anthropic.skilljar.com` and `academy.claude.com` pages — Anthropic's own course platforms — and exchanges Tutor requests through authenticated extension ports. Those two hosts are the entire list, checked in the extension's service worker; no other page, subdomain, or frame can reach the Tutor transport. On Academy the check also requires a course unit path, so the learner's account, settings and catalog pages are outside it. On an assessment page the Tutor is additionally told not to answer questions, and the lesson body — which is where answer choices live — is withheld from its context entirely. Its SDK object and extension-port payloads are not exposed to the course page's main JavaScript world. The visible Tutor UI remains in the shared page DOM, however, so course-page scripts may observe keyboard events and rendered questions or answers. The packaged source build disables the SDK's unused eager network and host-storage initialization, and no Puter authentication or AI request is made until the user sends a cloud Tutor message. The first such use opens a free Puter sign-in. Puter's HTTPS sign-in returns its result to the opener through browser window messaging, so the course page may also observe that authentication event while sign-in is happening; SkillBridge stores the accepted token only in extension storage. Baseline page translation never invokes Puter, Claude, or the Tutor model: it uses curated dictionaries, the local cache, Google Translate, deterministic protected-term restoration, and structure-preserving HTML reconciliation. Translation Refinement is a separate, optional feature that is off by default and has its own consent: when it is explicitly enabled AND consented, already-translated paragraphs near what you are reading may additionally be sent to the model you selected for post-editing, up to a per-page limit. Turning it off, or withdrawing that consent, stops any request already in flight. Selecting the local or off Tutor engine prevents Tutor messages from going to Puter.

### Data Stored Locally by the Unpublished Source Build
...
- **AI Tutor chat history (IndexedDB)** — your question, the tutor's answer, the language, the lesson heading, a timestamp, the lesson URL, and local conversation-grouping metadata are stored on your device so the sidebar can show past conversations. You can delete one conversation, clear all Tutor history, or explicitly download a client-side JSON export. The saved history is never added to a later Tutor request; only that request's active question and disclosed lesson context go to Puter or to your selected local server.
...
The unpublished source build does not transmit course text to YouTube. Auto-subtitles configure an existing embedded player and send it player-control messages; the extension requests no YouTube host permission.
...
| `academy.claude.com/courses` (content-script match)       | Translate lesson pages on Anthropic's Claude Academy. Scoped to course routes, so the learner's account, settings and catalog pages are outside the extension's reach. On assessment pages the answer choices are excluded from translation, so their text is never sent anywhere; where the page is already published in one of Academy's own locales, SkillBridge preserves the official copy: for locales whose script makes English residue identifiable (Korean, Japanese, Chinese) only that residue is sent, and where residue cannot be told apart from official copy (Spanish, French) nothing is sent at all |
| `academy.claude.com/*/courses` (content-script match)     | The same course routes under a locale prefix, which is how Academy serves its non-English locales                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
The unpublished source build lets the user run the AI Tutor against a local OpenAI-compatible server (for example Ollama) instead of the cloud. When that engine is selected:
...
### Unpublished Source-Build Retention
