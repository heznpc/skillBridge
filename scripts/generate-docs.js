...
// --- site/src/data/build-info.ts ---
const astroBuildInfoPath = path.join(ROOT, 'site', 'src', 'data', 'build-info.ts');
const astroBuildInfo = {
  version,
  languageCount: translatedCount,
  supportedLanguages: uniqueAvailable,
};
fs.mkdirSync(path.dirname(astroBuildInfoPath), { recursive: true });
fs.writeFileSync(
  astroBuildInfoPath,
  `/** Generated by scripts/generate-docs.js. */\nexport const buildInfo = ${JSON.stringify(astroBuildInfo, null, 2)} as const;\n`,
  'utf8',
);
...
console.log(`    - ${path.relative(ROOT, astroBuildInfoPath)}`);
