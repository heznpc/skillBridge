...
| Gate | Catches |
|---|---|
| `check:i18n` / `check:dict-coverage` | Key/section parity across all 13 dictionaries, `_meta.version` sync with the manifest |
...
| `tests/protected-terms.test.js` | Real-dictionary regression: ordinary prose in **all 13 locales** must survive protected-term restoration uncorrupted (incl. the CJK foreign-name interpunct guard — "克洛德·莫奈" stays intact) |
